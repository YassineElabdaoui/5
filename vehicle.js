...
// this funcction finds the projection of a on b
...
class Vehicle {
  ...
  */
  ...
  // this shadow function is used to create a shadow of the vehicle
  // the shadow is this.imageVaisseau but with a lower opacity
  // the image in shadow must be the same size as the original image
  // and turn with same as the original image
  
   shadow() {
    push();

    // Draw the shadow with black color
    tint(0, 0, 0, 100); // Black color with 80% transparency (adjust as needed)
    imageMode(CENTER);
    push();
    translate(this.pos.x + 5, this.pos.y + 5);
  ...
  border() {
    // Check if the vehicle is going outside the canvas
    if (this.pos.x > width - this.r_pourDessin) {
      this.pos.x = width - this.r_pourDessin;
      this.vel.x *= -1; // Reverse the x-direction velocity
    } else if (this.pos.x < this.r_pourDessin) {
      this.pos.x = this.r_pourDessin;
      this.vel.x *= -1; // Reverse the x-direction velocity
    }

    if (this.pos.y > height - this.r_pourDessin) {
      this.pos.y = height - this.r_pourDessin;
      this.vel.y *= -1; // Reverse the y-direction velocity
    } else if (this.pos.y < this.r_pourDessin) {
      this.pos.y = this.r_pourDessin;
      this.vel.y *= -1; // Reverse the y-direction velocity
    }
  }

  shoot() {
    // Create a new instance of the Ball class at the current position of the vehicle
    let bullet = new Ball(this.pos.x, this.pos.y, this.vel.copy());

    // Add the bullet to a global array (you may need to declare this array)
    bullets.push(bullet);
  }
}
